        ...
        <div class="col-12 col-lg-6">
          <div class="contact-info">
            ...
            <p class="contact-container-description">
              {{ $t('Contact.contact_des') }}
            </p>
            <p class="contact-container-description">
              {{ $t('Contact.contact_des2') }}
            </p>
            <p class="contact-container-description">
              {{ $t('Contact.contact_des3') }}
            </p>
            ...
            <div class="contacts-details">
              <div class="row contact-ways">
                ...
                <div class="col-12 col-md-10">
                  <div class="contact-title fs-5">
                    {{ $t('Contact.Phone') }}
                  </div>
                  <div class="contact-des phone_number fs-5">
                    +971 50 372 5888
                  </div>
                  <div class="contact-des phone_number fs-5">
                    +90 554 710 50 34
              ...
              <div class="row contact-ways">
                ...
                <div class="col-12 col-md-10">
                  ...
                  <div class="contact-des fs-5 location">
                    Port Saeed Business Avenue Tower Office 916 Dubai/UAE
                  </div>
                  <div class="contact-des fs-5 mt-2 fw-bold location">
                    Kazlıçeşme, Yedimavi Blok G2 Ofis 6,
                    Zeytinburnu/İstanbul/Turkiye
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="col-12 col-lg-6">
          <div class="contact-form">
            <form @submit.prevent="onSubmit">
              <h2 v-if="success" class="btn btn-success m-2 mb-4 text w-100">
                {{ fed_back_success }}
              </h2>
              ...
              <div class="mb-5">
                <input
                  ...
                  id="email"
                  name="email"
                  :placeholder="$t('Contact.Form_E-Mail')"
                />
              ...
              <div class="mb-5">
                <input
                  ...
                  id="phone_number"
                  name="phone_number"
                  :placeholder="$t('Contact.Form_Phone_Number')"
                />
                <span v-if="errors.phone_number" class="errors">{{
                  errors.phone_number
                }}</span>
              </div>
              <div class="mb-5">
                <textarea
                  class="form-control"
                  :placeholder="$t('Contact.Form_Write_YourMessage_Here')"
                  v-model="message"
                  id="message"
                  name="message"
                ></textarea>
                <span v-if="errors.message" class="errors">{{
                  errors.message
                }}</span>
              </div>
              <button v-if="!loading" type="submit" class="">
                {{ $t('Contact.Form_Send_Now') }}
              </button>
  ...
  methods: {
    async onSubmit() {
      ...
      try {
        ...
        await axios
          .post(
            'https://staging.zeintur.namaatests.com/api/v1/ztr/settings/contact-us',
            data
          )
          .then((response) => {
            ...
            if (response.data.code == 422) {
              this.success = false
              this.errors.name = response.data.errors.invalid_fields.find(
                (e) => e.field == 'name'
              )?.message
              this.errors.email = response.data.errors.invalid_fields.find(
                (e) => e.field == 'email'
              )?.message
              this.errors.message = response.data.errors.invalid_fields.find(
                (e) => e.field == 'message'
              )?.message
              this.errors.phone_number =
                response.data.errors.invalid_fields.find(
                  (e) => e.field == 'phone_number'
                )?.message
            }
            this.loading = false
          })
